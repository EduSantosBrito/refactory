...
import { Vector2, Vector3, Raycaster, Plane, MathUtils, Color, Quaternion } from "three";
...
import { slideMovementSpatial, type SpatialHash } from "./collision";
import { WORLD_RADIUS } from "./WorldScene";
...
const BARBARA_THEME_COLOR = "#ffc000";
...
function sampleTerrainNormal(x: number, z: number, target: Vector3): Vector3 {
  ...
  return target.set(left - right, TERRAIN_NORMAL_SAMPLE * 2, back - front).normalize();
}
...
function horizontalStepForSurfaceDistance(
  ...
  const sampleY = surfaceHeightAt(x + dirX * sampleDistance, z + dirZ * sampleDistance);
...
export function PlayerController({
  ...
  const markerData = useRef({ active: false, x: 0, z: 0, valid: true, elapsed: 0 });
  ...
  const handlePointerDown = useCallback(
    (e: PointerEvent) => {
      ...
      if (raycaster.ray.intersectPlane(groundPlane, intersectPoint)) {
        ...
        const valid = !isInWater(tx, tz) && Math.sqrt(tx * tx + tz * tz) < WORLD_RADIUS;
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    zoom.current = MathUtils.clamp(zoom.current + dir * ZOOM_STEP, ZOOM_MIN, ZOOM_MAX);
  }, []);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    edgeDir.current.x =
      e.clientX < EDGE_PAN_SIZE ? -1 : e.clientX > innerWidth - EDGE_PAN_SIZE ? 1 : 0;
    edgeDir.current.z =
      e.clientY < EDGE_PAN_SIZE ? -1 : e.clientY > innerHeight - EDGE_PAN_SIZE ? 1 : 0;
  }, []);

  /* ── Cursor leave / enter detection ────────────────────── */
  ...
  useFrame((_, delta) => {
    ...
    if (movingRef.current) {
      ...
      if (dist < ARRIVE_THRESHOLD) {
        ...
        if (diff < -Math.PI) diff += Math.PI * 2;
        group.rotation.y += diff * MathUtils.clamp(ROTATION_SPEED * delta, 0, 1);
      }
    }
    ...
    const movedDistance = Math.hypot(pPos.x - prevX, pPos.y - prevY, pPos.z - prevZ);
    const walkLoopAudio = walkLoopAudioRef.current;
    if (walkLoopAudio) {
      const targetVolume = movedDistance > MOVEMENT_EPSILON ? walkLoopVolume : 0;
      const fadeSpeed = targetVolume > 0 ? WALK_LOOP_FADE_IN_SPEED : WALK_LOOP_FADE_OUT_SPEED;
      ...
      if (targetVolume === 0 && !walkLoopAudio.paused && walkLoopAudio.volume < 0.001) {
        walkLoopAudio.pause();
        walkLoopAudio.currentTime = 0;
        walkLoopAudio.volume = 0;
      }
    ...
    if (tiltGroup) {
      ...
      localTerrainNormal.copy(terrainNormal).applyQuaternion(yawQuaternionInverse);
    ...
    /* ── Edge panning ──────────────────────────────────────── */
    const ep = edgeDir.current;
    if (ep.x !== 0 || ep.z !== 0) {
      panOffset.current.x += ep.x * EDGE_PAN_SPEED * delta;
      panOffset.current.z += ep.z * EDGE_PAN_SPEED * delta;
    }
    ...
    const lookX = pPos.x + panOffset.current.x;
    const lookZ = pPos.z + panOffset.current.z;

    cameraSmooth.current.x = MathUtils.lerp(cameraSmooth.current.x, lookX, CAMERA_LERP * delta);
    cameraSmooth.current.y = MathUtils.lerp(
      cameraSmooth.current.y,
      pPos.y * CAMERA_HEIGHT_TRACK,
      CAMERA_LERP * delta,
    );
    cameraSmooth.current.z = MathUtils.lerp(cameraSmooth.current.z, lookZ, CAMERA_LERP * delta);
    ...
    camera.lookAt(cameraSmooth.current.x, cameraSmooth.current.y, cameraSmooth.current.z);
    ...
    if (marker && markerMat) {
      if (md.active) {
        ...
        if (md.elapsed < MARKER_DURATION) {
          ...
          markerMat.color.copy(md.valid ? MARKER_COLOR_VALID : MARKER_COLOR_INVALID);
        } else {
        ...
        <group ref={tiltGroupRef}>
          <Suspense fallback={null}>
            <Character
              ...
              labelColor={BARBARA_THEME_COLOR}
              chatBubble={chatBubble}
              onAnimationComplete={animation === "Wave" ? handleWaveComplete : undefined}
            />
          </Suspense>
        </group>
        {/* Blob shadow */}
