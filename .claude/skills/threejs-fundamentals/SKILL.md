...
renderer.shadowMap.type = THREE.PCFShadowMap; // Soft by default since r178
...
> **Note:** `THREE.Clock` is deprecated since r179. Use `THREE.Timer` instead.
> Timer is in core — no addon import needed. Call `timer.update()` each frame before reading values.
